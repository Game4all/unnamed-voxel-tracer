...
// API:        GL_VERSION_4_5
// Profile:    core
// Extensions: GL_ARB_bindless_texture, GL_ARB_compute_variable_group_size
//

//
// This file was generated with the following command line:
// generator D:\Dev\zig-opengl\bin\Debug\net6.0\generator.dll .\OpenGL-Registry\xml\gl.xml ./gl.zig GL_VERSION_4_5 GL_ARB_bindless_texture GL_ARB_compute_variable_group_size
//
...
pub const GL_ARB_compute_variable_group_size = struct {
...
pub fn dispatchComputeGroupSizeARB(_num_groups_x: GLuint, _num_groups_y: GLuint, _num_groups_z: GLuint, _group_size_x: GLuint, _group_size_y: GLuint, _group_size_z: GLuint) callconv(.C) void {
    return @call(.always_tail, function_pointers.glDispatchComputeGroupSizeARB, .{_num_groups_x, _num_groups_y, _num_groups_z, _group_size_x, _group_size_y, _group_size_z});
}

pub fn load(load_ctx: anytype, get_proc_address: fn(@TypeOf(load_ctx), [:0]const u8) ?FunctionPointer) !void {
    var success = true;
    if(get_proc_address(load_ctx, "glDispatchComputeGroupSizeARB")) |proc| {
        function_pointers.glDispatchComputeGroupSizeARB = @ptrCast(proc);
    } else {
        log.err("entry point glDispatchComputeGroupSizeARB not found!", .{});
        success = false;
    }
    if(!success)
        return error.EntryPointNotFound;
...
const function_signatures = struct {
    ...
    const glDispatchComputeGroupSizeARB = fn(_num_groups_x: GLuint, _num_groups_y: GLuint, _num_groups_z: GLuint, _group_size_x: GLuint, _group_size_y: GLuint, _group_size_z: GLuint) callconv(.C) void;
};
...
const function_pointers = struct {
    ...
    var glDispatchComputeGroupSizeARB: *const function_signatures.glDispatchComputeGroupSizeARB = undefined;
};
